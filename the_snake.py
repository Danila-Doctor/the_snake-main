...
def show_game_over(score):
    ...
    hint_text = font.render("Подсказка: введите 'Admin' в ответ на первый вопрос", True, (0, 0, 255))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r or key_char == 'к':
                    waiting = False
                    return True
                elif event.key == pygame.K_q or key_char == 'й':
                    pygame.quit()
                    raise SystemExit
...
def ask_question(question, correct_answer):
    ...
    while True:
        ...
        screen.blit(question_surf, (SCREEN_WIDTH // 2 - question_surf.get_width() // 2, SCREEN_HEIGHT // 2 - 60))

        input_surf = font_normal.render(f"> {input_text}_", True, TEXT_COLOR)
        screen.blit(input_surf, (SCREEN_WIDTH // 2 - input_surf.get_width() // 2, SCREEN_HEIGHT // 2 - 10))

        if error_count > 0:
            error_surf = font_small.render("Ответ неверный", True, ERROR_COLOR)
            screen.blit(error_surf, (SCREEN_WIDTH // 2 - error_surf.get_width() // 2, SCREEN_HEIGHT // 2 + 30))

        if error_count >= 3:
            hint_surf = font_small.render(f"Правильный ответ: {correct_answer}", True, HINT_COLOR)
            screen.blit(hint_surf, (SCREEN_WIDTH // 2 - hint_surf.get_width() // 2, SCREEN_HEIGHT // 2 + 60))
